...
import { convertPx } from 'src/utils/convert-px';
import { Panel } from './panel';
import { devWarning } from 'src/utils/dev-log';
import { traverseReactNode } from 'src/utils/traverse-react-node';
...
export type IndexBarProps = {
  ...
  // custom component will have extra top area
  extraHeight?: number;
  ...
  const { run: checkActiveIndex } = useThrottleFn(
    () => {
      ...
      for (let i = 0; i < elements.length; i++) {
        ...
        if (
          panel.getBoundingClientRect().top + panel.clientHeight - titleHeight - props.extraHeight >
          scrollTop
        ) {
